...
async def get_latest_gnss_UTC_Time():
    query = "SELECT utcime FROM gnss_data_GGA ORDER BY id DESC LIMIT 1"
    result = await execute_query(query)
    return result[0][0] if result else None
...
async def get_latest_gnss_Number_of_Satellites():
    query = "SELECT number_of_satellites FROM gnss_data_GGA ORDER BY id DESC LIMIT 1"
    result = await execute_query(query)
    return result[0][0] if result else None
...
async def get_latest_gnss_Date():
    query = "SELECT date FROM gnss_data_RMC ORDER BY id DESC LIMIT 1"  
    result = await execute_query(query)
    return result[0][0] if result else None
